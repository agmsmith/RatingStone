class CreateLedgerAndLinkBases < ActiveRecord::Migration[6.0]
  def change
    create_table :ledger_bases, force: false, comment: "Ledger objects base class and record.  Don't force: cascade deletes since this is a write-once ledger where usually nothing gets deleted (except during points expired object garbage collection)." do |t|
      ...
      t.boolean :expired, default: false, comment: "When true (usually set by the weekly award ceremony prior to garbage collection at the next ceremony), the record will be deleted soon.  Other than displaying a warning to the user when viewing this object (suggesting that they spend some points to revive it), doesn't affect how the record is treated."
