...
class LedgerDeleteTest < ActiveSupport::TestCase
  test "deleting records" do
    ...
    amended_lbase.save!
    second_lbase = LedgerPost.new(
      creator_id: 0,
      subject: "Second Test",
      content: "A second LedgerBase record, actually a post.",
    )
    second_lbase.save!
    link_original_second = LinkBase.new(
      creator_id: 0,
      parent: original_lbase,
      child: second_lbase,
      rating_points_spent: 1,
      rating_points_boost_parent: 0.25,
      rating_points_boost_child: 0.5,
      rating_direction_parent: "U",
      rating_direction_child: "D",
    )
    ...
    ledger_delete = LedgerDelete.mark_records(
      records_to_delete,
      true,
      root_user,
      "Some Context",
    ...
    ledger_undelete = LedgerDelete.mark_records(
      records_to_undelete,
      false,
      root_user,
      "Some Other Context",
  ...
  test "delete needs permission" do
    assert_raise(RatingStoneErrors) do
      LedgerDelete.mark_records(
        [ledger_posts(:lpost_one)],
        true,
        ledger_users(:member_user),
        "Testing delete from wrong user.",
      )
    end
    assert_raise(RatingStoneErrors) do
      LedgerDelete.mark_records(
        ...
        false,
        ledger_users(:member_user),
        "Testing undelete on someone else's thing.",
      )
    end
    ldelete = LedgerDelete.mark_records(
      [ledger_posts(:lpost_two)],
      true,
      ledger_users(:member_user),
      "My Context",
    ...
    lundelete = LedgerDelete.mark_records(
      [ledger_posts(:lpost_two)],
      false,
