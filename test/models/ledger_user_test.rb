...
class LedgerUserTest < ActiveSupport::TestCase
  ...
  test "Weekly bonus points should accumulate with fading" do
    ...
    # ceremony happens.
    lbonus_explanation = LedgerPost.create!(
      creator_id: 0,
    ...
    lbonus_link = LinkBonus.create!(
      creator_id: 0,
      bonus_explanation: lbonus_explanation,
      bonus_user: luser,
      bonus_points: 10,
      expiry_ceremony: 8,
    ...
    assert(
      lbonus_link.approved_parent && lbonus_link.approved_child &&
            !lbonus_link.deleted,
      "Bonus link should be fully approved.",
    )
    luser.update_current_points
    assert_in_delta(
      regular_points * 0.97 + 1.0,
      luser.current_meh_points,
      0.0000001,
    )
    ...
    luser.update_current_points
    assert_in_delta(
      (regular_points * 0.97 + 1.0) * 0.97 + 10.0,
      luser.current_meh_points,
      0.0000001,
    )
    LedgerAwardCeremony.start_ceremony
  ...
  test "Shouldn't be able to add a second unique bonus" do
    ...
    lbonus_link = LinkBonusUnique.create!(
      creator_id: 0,
      bonus_explanation: lpost,
      bonus_user: luser,
      bonus_points: 1,
      expiry_ceremony: 8,
      approved_parent: true,
    ...
    assert(
      lbonus_link.approved_parent && lbonus_link.approved_child &&
            !lbonus_link.deleted,
      "Unique Bonus link should be fully approved.",
    )
    luser.update_current_points
    ...
    lbonus_second_link = LinkBonusUnique.create(
      creator_id: 0,
      bonus_explanation: lpost,
      bonus_user: luser,
      bonus_points: 2,
    ...
    assert_equal(
      1,
      lbonus_second_link.errors.size,
      "Should fail to save a second unique bonus.",
    )
    assert_equal(
      "Creating a LinkBonusUnique which isn't unique - " \
    ...
    LedgerDelete.mark_records(
      ...
      LedgerUser.find(0),
      "Should work.",
      "Testing deletion of First Bonus.",
    )
    luser.reload
    assert_in_delta(
      0.0,
    ...
    assert_raise(RatingStoneErrors, "Undelete unique bonus") do
      LedgerDelete.mark_records(
        ...
        LedgerUser.find(0),
        "Should fail",
        "Testing undeletion of First Bonus.",
      )
    end
    luser.update_current_points
    assert_in_delta(
    ...
    assert_equal(
      ...
      LedgerDelete.mark_records(
        ...
        LedgerUser.find(0),
        "Should work.",
        "Deleting the second unique bonus.",
      ).class,
      "Delete didn't work?",
    )
  ...
  test "See if bonus points get received and recalculated properly" do
    ...
    luser.update_current_points
    assert_in_delta(
      10.0 * 0.97 * 0.97 * 0.97 + 10.0 * 0.97 + 10.0,
      luser.current_meh_points,
      0.0000001,
    )
    assert_in_delta(0.0, luser.current_up_points, 0.0000001)
